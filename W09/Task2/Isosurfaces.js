function Isosurfaces( volume, isovalue )
{
        ...
        for ( var y = 0; y < volume.resolution.y - 1; y++ )
        {
            for ( var x = 0; x < volume.resolution.x - 1; x++ )
            {
                ...
                for ( var j = 0; lut.edgeID[index][j] != -1; j += 3 )
                {
					...
					//分割点を宣言(どの頂点間におくか)  isovalue: ratio parameter
    ...
    {
		//get vertex id
		var v0_index = (v0.z * volume.resolution.x * volume.resolution.y) + (v0.y * volume.resolution.x) + v0.x;
		var v1_index = (v1.z * volume.resolution.x * volume.resolution.y) + (v1.y * volume.resolution.x) + v1.x;
		//select vertex
		var s0 = volume.values[ v0_index ][0];
		var s1 = volume.values[ v1_index ][0];
		//s0 < s < s1 ration parameter
		var t = (s-s0)/(s1-s0);
